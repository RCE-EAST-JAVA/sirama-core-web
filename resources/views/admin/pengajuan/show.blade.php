<x-dashboard-layout title="Detail Pengajuan" :pageTitle="'Pengajuan #' . $pengajuan->id">
    ...
    <div class="max-w-4xl space-y-6">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6">
            ...
            <ul class="space-y-2">
                @foreach($pengajuan->lokasi_dokumen as $i => $filePath)
                <li class="flex items-center gap-2 text-sm bg-gray-50 rounded-lg px-3 py-2">
                    <svg class="w-4 h-4 text-red-500 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4zm2 6a1 1 0 011-1h6a1 1 0 110 2H7a1 1 0 01-1-1zm1 3a1 1 0 100 2h6a1 1 0 100-2H7z" clip-rule="evenodd" />
                    </svg>
                    <span class="truncate text-gray-700">{{ basename($filePath) }}</span>
                    <a href="{{ route('dokumen.softfile', [$pengajuan, $i]) }}"
                       class="ml-auto text-xs text-brand-600 hover:text-brand-800 font-medium px-2 py-1 rounded hover:bg-brand-50 transition-colors">
                        Unduh
                    </a>
                </li>
                @endforeach
            ...
            @else
            <p class="text-xs text-gray-400">Belum ada softfile.</p>
            @endif
        </div>
